...
class _ChildDashboardCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final progressAsync = ref.watch(
      StreamProvider.family<ProgressModel, String>(
        (ref, id) => ref.read(progressRepositoryProvider).watchProgress(id),
      )(child.id),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _Last7DaysChart(childId: child.id),
          ],
        ),
      ),
    );
...
class _Last7DaysChart extends ConsumerWidget {
  ...
  const _Last7DaysChart({required this.childId});
